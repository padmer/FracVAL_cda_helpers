...
fold_name="Dfvals_ 1.8"

#Change depending on spacings required
spacings="seq(0.8,1.45,0.05)"
...
run_cda_single<-function(line,numvals_start,numvals_stop){
  ...
  for (i in numvals_start:numvals_stop){
    ...
    indicom=paste("meanvals",i,"<-get_aggregate_mean_oa(clustersa[[",i,"]],length(clustersa[[i]]))",sep="")
...
#run_functions
line=paste("files<-Sys.glob(\"",wd_fold,"/",fold_name,"/","*.dat\")","
...
for (i in 1:length(spac)){
  clustersa[[length(clustersa)+1]]<-adjust_spacing_many(clusters,spac[i])
}

",sep="")

#Choose whether to seperate the jobs into two batches or not and run them
numvals=length(eval(parse(text=spacings)))
if (numvals>15){
...
saveRDS(meanvalsabs,paste(substr(fold_name,1,(nchar(fold_name)-4)),"spacings","abs",".RDS",sep=""))
saveRDS(meanvalsext,paste(substr(fold_name,1,(nchar(fold_name)-4)),"spacings","ext",".RDS",sep=""))
